...
前文厘清了 Runtime 的价值与 Harness 的落地逻辑，本章将聚焦核心问题：Runtime 如何从工程层面，将概率性的模型推理，转化为确定性的系统运行，实现 Agent 从 Demo 到生产的质变？
...
这是过去一年国外 Runtime 讨论最多的话题之一。因为越来越多 Agent 已经可以操作浏览器、邮件、数据库，并且具有支付能力等，它们开始真正影响现实世界。因此，传统的软件权限模型已经不够用了。未来 Runtime 更像操作系统（Operating System），所有能力都必须经过系统规则（Policy）。
...
## 第七章 ｜ 行业实践：为什么所有 AI 公司都开始构建 Runtime

过去两年，AI Agent 的产业化发展速度，远超行业普遍预期。头部厂商都在加速落地 Agent 相关能力：OpenAI 推出了 ChatGPT、Responses API 和 Computer Use；Anthropic 持续强化 Claude Code 与企业级能力；Google 将 AI 深度整合进云平台；Microsoft 围绕 Copilot 与 Multi-Agent 持续布局等。

从表层产品形态来看，各家的发展路径看似截然不同：有的专注于 Coding，有的专注于 Research；有的强调 Browser，有的强调 Multi-Agent；有的提供开发平台，有的直接交付 AI 产品。

但如果站在 Runtime Engineering 的视角重新观察，就会发现一个更关键的行业趋势：几乎所有领先的 AI 公司，都在投入大量资源构建属于自己的 Agent Runtime。唯一的区别在于：每家公司认为，Agent 最大的工程挑战是什么，因此优先建设 Runtime 的哪一种能力。

从这个角度来看，它们虽然选择了不同的产品路线，却都在回答同一个问题：如何让 Agent 从一次性的模型调用，演变为能够持续、可靠、安全运行的软件系统。

### 7.1 OpenAI：降低 Agent 开发门槛

OpenAI 解决的核心问题并不是“如何让模型更聪明”，而是“如何让开发者更容易构建 Agent”。

过去，开发一个 Research Agent，开发者通常需要自己维护浏览器、Python 执行环境、文件系统、工具调用、后台任务、状态管理等一整套运行能力。如今，随着 Responses API、Computer Use、Code Interpreter、Background Tasks 等能力的推出，这些原本需要自行开发的 Runtime 能力，越来越多地被平台直接提供。

如果 Runtime 变成平台（Platform），那么所有 Agent 都可以复用同样的基础设施，避免重复造轮子，仅专注于业务：

```Plain
Runtime 平台（Runtime Platform）
...
OpenAI 希望开发者摆脱“Agent 应该如何运行”的困境，转而关注“Agent 要完成什么任务”。因此，OpenAI 的 Runtime 战略可以概括为：Runtime 平台化（Runtime as a Platform）。

### 7.2 Anthropic：让 Agent 值得信任

如果说 OpenAI 以**开发效率**为核心，Anthropic 的 Runtime 建设则始终围绕**可信性与安全性**展开。

观察 Claude Code 的产品设计不难发现，其核心逻辑从不盲目依赖模型输出的结果。代码修改完成后，会主动重读文件校验执行效果；执行高危终端命令前，会强制触发用户授权机制；调用计算机操作能力时，会严格划定权限边界、规避越权风险；运行长周期任务时，会持续巡检任务状态、及时修正异常偏差。这一系列严谨的约束与校验设计，反映了 Anthropic 的工程理念：模型一定会犯错。

因此，模型仅负责输出决策方案，而 Runtime 承担核心的校验、约束、纠错与异常恢复职责，形成“模型决策、Runtime 兜底”的安全架构。

Anthropic 的 Runtime 战略就是打造一套可信的 Runtime，让 Agent 值得信任。

### 7.3 Google：让 Runtime 成为云基础设施

Google 思考 Runtime 的方式，与 OpenAI 和 Anthropic 都有所不同。

相比起“一个 Agent 如何完成任务”，Google 更关心如何同时运行成千上万个 Agent。因此，它天然会从云基础设施的角度思考 Runtime。例如：
...
Google 希望构建的是一套能够承载海量 Agent 的运行平台。因此，它的 Runtime 战略可以总结为：让 Runtime 成为云基础设施（Runtime as Cloud Infrastructure）。

### 7.4 Microsoft：打造可组合 Runtime

凭借深厚的传统软件工程积累，微软对 AI Agent 的架构认知更贴合成熟企业软件体系，核心强调协同性与模块化。Microsoft 认为 Agent 不是孤立的功能工具，应该是可拆解、可组合、可协同的软件组件。无论是 AutoGen 多 Agent 框架，还是 Copilot Studio 开发平台，微软的技术布局始终围绕三大核心：多 Agent 协同协作、多工具智能编排、多流程串联成完整的企业业务系统。

相较于“如何从零搭建单个优质 Agent”，微软更关注如何系统化组织、调度、管理海量 Agent，实现 AI 能力与企业现有软件生态的深度融合。因此，微软的 Runtime 战略可定义为：打造可组合 Runtime（Composable Runtime）。

### 7.5 AI 软件工程的核心竞争力正在改变

过去，一个优秀的软件团队，核心竞争力通常来自更好的架构、更高质量的代码、更完善的测试。

未来，这些能力依然重要，但还不够。因为团队还需要回答新的问题：
...
这些问题已经超出了传统软件工程的范畴。真正能够解决它们的，是 Runtime Engineering。因此，未来团队之间的竞争，也会逐渐从“谁拥有更好的模型”，转变为“谁拥有更成熟的 Runtime”。模型能力越来越趋同，Runtime 能力却会持续形成差异。

### 小结

如果回顾软件的发展历史，我们可以看到软件栈一直在不断演进。最早的软件栈可以简化为：
...
这里最大的变化是，模型第一次成为基础设施的一部分。而 Runtime，则成为连接业务与模型之间的新抽象层。未来的软件，不再直接建立在操作系统之上，而是建立在 Runtime 之上。

未来，随着行业基础模型能力逐步趋同、同质化，Runtime 将复刻数据库、操作系统、云平台的发展路径，成为 AI 软件产业全新的底层基础设施，成为企业核心的竞争壁垒。

AI 产业的竞争，已正式从模型能力内卷，迁移至 Runtime 工程能力的比拼。

## 第八章 ｜Runtime 未来：AI 软件工程的下一个十年

回顾过去几十年的软件发展，我们会发现一个有趣的规律。每当计算范式发生变化，软件工程都会重新定义自己的核心抽象：

大型机时代，软件围绕程序（Program）构建；

互联网时代，软件围绕服务（Service）构建；

云计算时代，软件围绕平台（Platform）构建；

而 AI Agent 的出现，正在推动软件进入新的阶段。

未来的软件，不再只是静态执行预先编写好的逻辑，而是能够自主规划、持续运行、主动调用工具，并与真实世界持续交互。这意味着，软件第一次拥有了“行为（Behavior）”。而管理这种行为，将成为下一代软件工程最重要的问题。

### 8.1 能力边界迭代：模型持续吞噬 Runtime 能力

过去两年，一个问题经常被讨论：随着基础模型（Foundation Model）越来越强，Runtime 会不会逐渐消失？

答案是：会，也不会。

诚然，越来越多过去属于 Runtime 的能力，正在被模型逐渐吸收。例如：

（1）任务规划（Planning）

早期 Agent 往往依赖外部 Workflow 或 Planner 完成任务拆解，而现在，模型已经能够在一次推理中生成相当复杂的执行计划。

（2）工具理解（Tool Understanding）

过去开发者需要精心设计 Tool Prompt，而今天的模型已经能够根据模式（Schema）自动理解工具能力，并选择合适的调用方式。

（3）短期记忆（Short-term Memory）

随着上下文窗口不断扩大，越来越多原本需要 Runtime 管理的会话状态，可以直接保存在上下文之中。

此外，代码生成、函数调用、甚至部分反思（Reflection）能力，也正在逐渐成为模型的内在能力。模型正在不断侵蚀 Runtime 的边界。这是一个不可逆的趋势。

### 8.2 不可替代的 Runtime 核心能力

然而，还有另一类能力，几乎不可能被模型完全替代。因为它们并不是推理能力，而是工程能力。

首先是状态（State）。模型可以理解状态，但状态本身属于外部世界。数据库、文件系统、浏览器、任务进度、用户权限、执行历史，都必须由 Runtime 持久化管理。

其次是验证（Verification）。模型可以判断一段代码“看起来正确”，但它无法仅凭推理确认：
...
第三是恢复（Recovery）。恢复不是一种智能，而是一套系统机制。检查点（Checkpoint）、回滚（Rollback）、重放（Replay）、重试（Retry）这些能力来源于分布式系统和工作流引擎，而不是语言模型。

第四是治理（Governance）。权限、安全、审计、资源预算、合规策略，都属于企业系统的管理范畴。模型可以遵守规则，但规则必须由 Runtime 定义和执行。

最后是可观测性（Observability）。日志、指标、成本分析、运行轨迹，这些信息不仅服务于模型，更服务于开发者、运维人员和企业，它们天然属于 Runtime。

因此，我们可以把未来的软件能力划分为两类：模型越来越擅长“思考（Thinking）”和 Runtime 永远负责“运行（Running）”。模型与 Runtime 并不是竞争关系，而是不断重新划分边界。

### 8.3 Runtime 将成为通用核心基础设施

原因很简单。

几乎所有 AI Agent 都需要解决相同的问题：状态管理、工具执行、权限控制、任务调度、错误恢复、结果验证、运行监控等。这些能力具有极强的通用性。就像今天几乎没有团队会重新开发自己的 TCP/IP 协议栈，也很少有人从零实现一个数据库内核。

未来，大多数团队也不会愿意反复实现一套 Agent Runtime。Runtime 将逐渐沉淀为基础设施。不同公司的竞争，将更多体现在 Runtime 的成熟度、可靠性和生态能力，而不仅仅是模型能力。

### 8.4 Agent 操作系统会出现吗

近年来，越来越多人开始讨论一个概念：Agent 操作系统。

这个名字很吸引人。但如果把操作系统理解为负责管理资源、调度执行、控制权限、维护生命周期的系统软件，那么，今天很多 Runtime 已经具备了相当一部分操作系统特征。它们管理的对象虽然不是传统进程，而是 Agent，但职责非常相似。

未来，我们很可能会看到 Runtime 进一步发展：一个 Runtime 同时运行多个 Agent；多个 Agent 共享记忆与工具；统一调度计算资源；统一管理权限和安全策略；统一记录运行日志和状态。届时，Runtime 与操作系统之间的界限将越来越模糊。

不过，我更倾向于把它称为 Agent Runtime，而不是 Agent OS。原因在于，操作系统强调的是底层资源管理，而 Runtime 更强调围绕 Agent 生命周期的运行管理。Agent OS 可以视为 Runtime 的一种实现形式，而不是 Runtime 本身。

### 8.5 Runtime Engineering 将成为新的软件工程

回到本文最初的问题：为什么 Runtime Engineering 会成为新的软件工程？

答案其实已经贯穿全文：

Prompt Engineering 教会模型如何回答问题；

Context Engineering 教会模型如何理解世界；

Harness Engineering 教会模型如何可靠运行。

而 Runtime Engineering，则试图回答一个更大的问题：如何构建能够长期稳定运行的 AI 软件系统。它讨论的不只是模型，不只是 Prompt，也不只是 Agent。它讨论的是“在一个由概率模型驱动的软件世界里，如何重新建立确定性的工程秩序”。

因此，Runtime Engineering 并不是 AI 工程中的一个新概念，而是软件工程在 AI 时代的一次自然延伸。

### 结语

软件工程的每一次进化，都是为了驯服新的系统复杂性。编程语言驯服了计算复杂性，分布式系统驯服了网络复杂性，云平台驯服了资源复杂性，而 Runtime Engineering 正在驯服智能运行的复杂性。

它不是 Prompt 的升级、不是框架的别名，而是一套全新的软件工程思想：将智能视为需要持续管控、校验、治理、迭代的运行对象。

未来的软件工程师，依然会编写代码，但核心工作将彻底升级：从「定义程序做什么」，升级为「定义智能如何运行」。模型赋予 AI 智能，Runtime 赋予 AI 成为可靠软件的能力，这就是 AI 软件工程的全新未来。
